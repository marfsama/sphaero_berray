...
public class JigsawPanel extends JPanel {
   ...
   */
  public void shuffle() {
    Piece[] pieces = new Piece[jigsaw.zOrder.size()];
    jigsaw.zOrder.toArray(pieces);
    jigsaw.shuffle(getWidth(), getHeight());
    repaint();
  }
  ...
  protected void paintComponent(Graphics g) {
    ...
    if (jigsaw.isFinished() && jigsaw.finishedImage != null) {
      Piece lastPiece = jigsaw.zOrder.get(0);
      int x = lastPiece.getPuzzleX();
      int y = lastPiece.getPuzzleY();
      g.drawImage(jigsaw.finishedImage, x, y, null);
    }
  }
  ...
  private void computePreferredSize() {
    JigUtil.ensureLoaded(jigsaw.getImage());
    int iWidth = jigsaw.getWidth();
    int iHeight = jigsaw.getHeight();
  ...
  private void finish() {
    ...
    ActionListener fader = new ActionListener() {
      ...
      public void actionPerformed(ActionEvent evt) {
        ...
        if (jigsaw.finishedImage != null) {
          jigsaw.finishedImage.flush();
        }
        jigsaw.finishedImage = Toolkit.getDefaultToolkit().createImage(
  ...
  private void grabPiece(MouseEvent e) {
    ...
    focusPiece = null;
    ListIterator<Piece> iter = jigsaw.zOrder.listIterator(jigsaw.zOrder.size());
  ...
  private void releasePiece() {
    ...
    if (result != null) {
      ...
      for (int i = 1; i < result.length; i++) {
        jigsaw.zOrder.remove(result[i]);
      }
      jigsaw.zOrder.add(newPiece);
      focusPiece = newPiece;
